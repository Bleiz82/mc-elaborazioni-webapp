...
export default function Communications() {
  ...
  return (
    <div className="h-[calc(100vh-8rem)] flex flex-col md:flex-row gap-6">
      ...
      <div className="w-full md:w-1/3 lg:w-1/4 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col overflow-hidden flex-shrink-0">
        <div className="p-4 border-b border-slate-200 dark:border-slate-700">
          <h2 className="text-lg font-bold text-slate-900 dark:text-slate-100 mb-4">Conversazioni</h2>
          <div className="relative">
            ...
            <input 
              ...
              className="w-full pl-9 pr-4 py-2 bg-slate-50 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-lg text-sm text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-sky-500 focus:bg-white dark:focus:bg-slate-800 transition-colors"
            />
          </div>
        </div>
              ...
              {filteredConvs.map(conv => (
                <button
                  ...
                  className={`w-full text-left p-4 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors flex items-start gap-3 ${
                    activeConvId === conv.id ? 'bg-sky-50/50 dark:bg-sky-900/20 border-l-2 border-sky-500' : 'border-l-2 border-transparent'
                  }`}
                >
                  <div className="w-10 h-10 rounded-full bg-slate-100 dark:bg-slate-700 flex items-center justify-center flex-shrink-0 overflow-hidden">
                    {conv.client_avatar ? (
      ...
      <div className="flex-1 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col overflow-hidden">
        {activeConvId ? (
          <>
            ...
            <div className="px-6 py-4 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between bg-slate-50/50 dark:bg-slate-900/50">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 flex items-center justify-center overflow-hidden shadow-sm">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-slate-50/30 dark:bg-slate-900/30">
              {loadingMessages ? (
                ...
                <div className="flex flex-col items-center justify-center h-full text-center space-y-3">
                  ...
                  <p className="text-slate-500 dark:text-slate-400 text-sm">Nessun messaggio in questa conversazione</p>
                </div>
              ) : (
                messages.map((msg) => {
                      ...
                      <div className={`max-w-[70%] ${isAdmin ? 'order-2' : 'order-1'}`}>
                        {isAdmin && msg.is_automated && (
                          <div className="flex items-center justify-end gap-1 mb-1 mr-1">
                            <span className="text-[10px] font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider">Inviato come AI</span>
              ...
              <form onSubmit={(e) => handleSendMessage(e, false)} className="flex items-end gap-3">
                <div className="flex-1 relative">
                  <textarea
                    ...
                    className="w-full bg-slate-50 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-xl px-4 py-3 text-sm text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-sky-500 focus:bg-white dark:focus:bg-slate-800 transition-all resize-none min-h-[50px] max-h-[150px]"
                    rows={2}
                    onKeyDown={(e) => {
                ...
                <div className="flex flex-col gap-2 flex-shrink-0">
                  ...
                  <button
                    ...
                    disabled={!newMessage.trim()}
                    className="h-10 px-4 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 border border-indigo-100 dark:border-indigo-800/50 rounded-lg flex items-center justify-center gap-2 hover:bg-indigo-100 dark:hover:bg-indigo-900/50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors font-medium text-sm"
          ...
          <div className="flex flex-col items-center justify-center h-full text-center space-y-4 bg-slate-50/50 dark:bg-slate-900/50">
            <div className="w-20 h-20 bg-white dark:bg-slate-800 rounded-full flex items-center justify-center shadow-sm border border-slate-100 dark:border-slate-700">
              <MessageSquare className="w-10 h-10 text-slate-300 dark:text-slate-600" />
            </div>
            <div>
              <h3 className="text-lg font-medium text-slate-900 dark:text-slate-100">Nessuna conversazione selezionata</h3>
              <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">Seleziona un cliente dalla lista per iniziare a chattare</p>
            </div>
          </div>
        )}
      </div>
