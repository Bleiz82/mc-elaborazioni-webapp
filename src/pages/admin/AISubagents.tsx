...
import { 
  Bot, Clock, Activity, CheckCircle2, AlertCircle, 
  Settings, Play, FileText, Database, Users, MessageSquare, Shield,
  ...
  useEffect(() => {
    ...
    const fetchStatus = async () => {
      ...
      if (docSnap.exists()) {
        ...
        setOrchestratorStatus(prev => ({ ...prev, ...data }));
      }
    };
    fetchStatus();
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      setAgents(prev => prev.map(agent => ({
        ...
        lastRun: logsData.find(l => l.subagent_id === agent.id)?.created_at || agent.lastRun
      })));
          ...
          <div className="flex items-center justify-between mb-8">
            ...
            <div className="flex items-center gap-3">
              <button 
                ...
                className={`px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-2 transition-colors ${orchestratorStatus.enabled ? 'bg-amber-500/20 text-amber-400 hover:bg-amber-500/30' : 'bg-emerald-500/20 text-emerald-400 hover:bg-emerald-500/30'}`}
              >
                {orchestratorStatus.enabled ? <><Pause className="w-4 h-4" /> Metti in Pausa</> : <><Play className="w-4 h-4" /> Attiva</>}
              </button>
              <button 
              ...
              >
                <RefreshCw className={`w-4 h-4 ${isOrchestratorRunning ? 'animate-spin' : ''}`} />
                Esegui Ciclo Ora
              </button>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            <div>
              ...
              <div className="flex items-center gap-2">
                <div className={`w-2.5 h-2.5 rounded-full ${orchestratorStatus.enabled ? 'bg-emerald-500 animate-pulse' : 'bg-slate-500'}`}></div>
                <span className="font-medium">{orchestratorStatus.enabled ? 'Attivo (In ascolto)' : 'In Pausa'}</span>
              </div>
            </div>
            <div>
              <p className="text-slate-400 text-sm mb-1">Ultimo Ciclo</p>
              <p className="font-medium">{new Date(orchestratorStatus.lastRun).toLocaleTimeString('it-IT', { hour: '2-digit', minute: '2-digit' })}</p>
            </div>
            <div>
            ...
            <div>
              ...
              <p className="font-medium text-sky-400 text-xl">{orchestratorStatus.actionsToday}</p>
            </div>
          </div>
        </div>
      </div>
            ...
            <div className="p-5 flex-1">
              ...
              <div className="flex items-center gap-2 mb-4">
                <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${
                  ...
                  {agent.status === 'active' ? 'Operativo' : agent.status === 'error' ? 'Errore' : 'In Pausa'}
                </span>
              </div>
